...
// Escena Three.js para el marco 3D de posicionamiento
let threeScene = null;
let threeCamera = null;
let threeRenderer = null;
let threeFrameBox = null;
let threeFrameAnimationId = null;
...
async function startScanner() {
    ...
    try {
        ...
        // Inicializar el marco 3D después de mostrar la pantalla
        setTimeout(() => {
            initThreeFrame();
        }, 100);
...
// =============================================================================
// MARCO 3D DE POSICIONAMIENTO CON THREE.JS
// =============================================================================

/**
 * Inicializa el marco 3D de posicionamiento
 */
function initThreeFrame() {
    ...
    // Crear cámara ortográfica para mejor precisión en 2D/3D
    ...
    // Crear el marco 3D (caja wireframe)
    createThreeFrameBox();
    
    // Iniciar animación del marco
    animateThreeFrame();
    
    console.log('Marco 3D inicializado');
}

/**
 * Crea el marco 3D de posicionamiento
 */
function createThreeFrameBox() {
    // Obtener dimensiones actuales del marco 2D
    const frameRect = scanFrame.getBoundingClientRect();
    ...
    // Crear geometría de caja (wireframe)
    const geometry = new THREE.BoxGeometry(frameWidth, frameHeight, 50);
    
    // Crear material wireframe con colores brillantes
    const material = new THREE.LineBasicMaterial({ 
        color: 0x00ff88,
        linewidth: 2
    });
    
    // Crear el marco usando EdgesGeometry para bordes más definidos
    const edges = new THREE.EdgesGeometry(geometry);
    threeFrameBox = new THREE.LineSegments(edges, material);
    
    // Posicionar en el centro del marco 2D
    ...
    threeFrameBox.position.set(
        centerX - containerRect.width / 2,
        centerY - containerRect.height / 2,
        0
    );
    
    threeScene.add(threeFrameBox);
    
    // Agregar indicadores de dimensión
    addDimensionLabels(frameWidth, frameHeight);
}

/**
 * Agrega etiquetas de dimensión al marco 3D
 */
function addDimensionLabels(width, height) {
    // Crear sprites para texto (simplificado)
    // Por ahora usamos geometry visual
}

/**
 * Actualiza la posición del marco 3D
 */
function updateThreeFrame() {
    if (!threeFrameBox) return;
...
function animateThreeFrame() {
    threeFrameAnimationId = requestAnimationFrame(animateThreeFrame);
    
    if (threeFrameBox) {
        // Efecto de pulso suave
        const time = Date.now() * 0.002;
        const pulse = Math.sin(time) * 0.1 + 1;
        threeFrameBox.scale.z = pulse * 0.5;
    }
...
function disposeThreeFrame() {
    if (threeFrameAnimationId) {
        cancelAnimationFrame(threeFrameAnimationId);
        threeFrameAnimationId = null;
    }
...
// =============================================================================
// EXTRACCIÓN DE COLORES DE LA IMAGEN
// =============================================================================

/**
 * Extrae los colores dominantes de la imagen capturada
 */
function extractColorsFromImage() {
    ...
    // Muestrear cada 10 píxeles para rendimiento
    for (let i = 0; i < data.length; i += 40) {
        const r = Math.round(data[i] / 32) * 32;
        const g = Math.round(data[i + 1] / 32) * 32;
        const b = Math.round(data[i + 2] / 32) * 32;
        
        // Ignorar pixels muy oscuros o muy claros
...
/**
 * Convierte color RGB a formato hexadecimal Three.js
 */
...
function generateRandomModel() {
    // Extraer colores de la imagen capturada
    extractColorsFromImage();
    
    // Si tenemos colores extraídos, usarlos
    let color;
    if (extractedColors.length > 0) {
        // Usar el color dominante
    ...
    // Crear geometría basada en las dimensiones del marco
    const frameRect = scanFrame.getBoundingClientRect();
    const aspectRatio = frameRect.width / frameRect.height;
    
    let geometry;
    // Ajustar geometría según proporción del marco
    if (aspectRatio > 1.2) {
        // Objeto ancho - usar caja rectangular
        geometry = new THREE.BoxGeometry(1.5 * aspectRatio, 1.5, 1.5);
    } else if (aspectRatio < 0.8) {
        // Objeto alto - usar cilindro
        geometry = new THREE.CylinderGeometry(0.8, 0.8, 2, 32);
    } else {
        // Objeto más cuadrado - usar esfera
        geometry = new THREE.SphereGeometry(1, 32, 32);
    }
    
    // Crear material con los colores extraídos o aleatorio
